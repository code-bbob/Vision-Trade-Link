...
export default function EMIDebtorTransactions() {
  ...
  // Initial load
  useEffect(() => {
    fetchPaginatedData(`transaction/emidebtortransaction/branch/${branchId}/`)
  }, [branchId])
  ...
  const handleSearch = async (e) => {
    ...
    setLoading(true)
    try {
      const url = `transaction/emidebtortransaction/branch/${branchId}/?search=${localSearchTerm}`
  ...
  const handleDateSearch = async (e) => {
    ...
    setLoading(true)
    try {
      const url = `transaction/emidebtortransaction/branch/${branchId}/?start_date=${startDate}&end_date=${endDate}`
      await fetchPaginatedData(url)
      setCurrentPage(1)
    } catch {
  ...
  const handleReferenceClick = (e, id) => {
    e.stopPropagation()
    navigate(`/mobile/sales/branch/${branchId}/editform/${id}`)
  }
