...
import { Search, ArrowLeft, Trash2, Plus, User, Check, ChevronsUpDown, PlusCircle } from "lucide-react";
...
export default function EMIDebtorsPage() {
  const api = useAxios();
    ...
    const fetchData = async () => {
      try {
        const response = await api.get(
          `transaction/emidebtors/branch/${branchId}/`
        );
  ...
  useEffect(() => {
    if (showNewDebtorDialog) {
      const fetchBrands = async () => {
        setLoadingBrands(true);
        try {
          const response = await api.get(`inventory/brand/branch/${branchId}/`);
  ...
  const handleDeleteSelected = async () => {
    ...
    try {
      await Promise.all(
        selectedDebtors.map((id) => api.delete(`transaction/emidebtors/${id}/`))
      );
      setDebtors((prev) =>
  ...
  const handleAddBrand = async () => {
    try {
      const resp = await api.post("inventory/brand/", newBrandData);
      setBrands((prev) => [...prev, resp.data]);
      setNewDebtorData((prev) => ({ ...prev, brand: resp.data.id.toString() }));
  ...
  const handleAddDebtor = async (e) => {
    e.preventDefault();
    try {
      const resp = await api.post("transaction/emidebtors/", newDebtorData);
      setDebtors((prev) => [...prev, resp.data]);
      setFilteredDebtors((prev) => [...prev, resp.data]);
      setNewDebtorData({ name: "", phone_number: "", due: 0, branch: branchId, brand: "" });
        ...
        <Card className="bg-gradient-to-b from-slate-800 to-slate-900 border-none shadow-lg">
          <CardContent className="overflow-x-auto p-0">
            <div className="grid grid-cols-12 gap-2 border-b border-slate-700 p-2 sm:p-4 text-slate-300 text-xs sm:text-sm font-medium">
              ...
              <div className="col-span-2 lg:col-span-2">Brand</div>
            ...
            {filteredDebtors.map((debtor) => (
              ...
              >
                ...
                <div className="col-span-2 lg:col-span-2 text-white text-sm lg:text-base">
                  {debtor.brand_name}
                </div>
                <div className="col-span-3 lg:col-span-3 text-white text-sm lg:text-base">
                  {debtor.phone_number}
                </div>
                <div className="col-span-2 lg:col-span-2 text-right text-white text-sm lg:text-base">
          ...
          <DialogContent className="bg-slate-800 text-white sm:max-w-[425px]">
            <DialogHeader>
              ...
              <DialogDescription className="text-slate-300">
                Enter the details of the new EMI debtor you want to add.
              </DialogDescription>
            </DialogHeader>
